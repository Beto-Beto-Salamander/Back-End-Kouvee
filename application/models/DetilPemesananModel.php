...
class DetilPemesananModel extends CI_Model{
    ...
    public function getall($id_trans){
        if($id_trans==null){
            $this->db->select(
            'DETIL_PEMESANAN.ID_DETIL_PEMESANAN,
            DETIL_PEMESANAN.ID_PEMESANAN,
            DETIL_PEMESANAN.ID_PRODUK, 
            PRODUK.NAMA_PRODUK,
            PRODUK.HARGA_BELI,
            PRODUK.SATUAN_PRODUK,
            DETIL_PEMESANAN.DATANG,
            DETIL_PEMESANAN.SUB_TOTAL_PEMESANAN,
            DETIL_PEMESANAN.JUMLAH_PESANAN')
    ...
    public function store($request) { 
        ...
        $result =  mysqli_query($conn,"SELECT COUNT(DISTINCT id_produk) as cnt FROM $this->detil WHERE id_produk = '$request->id_produk' AND id_pemesanan = '$request->id_pemesanan'");
        ...
        if ( $dup[0] == 0){
            ...
            mysqli_query($conn,"UPDATE $this->detil SET jumlah_pesanan = '$request->jumlah_pesanan' WHERE id_pemesanan = '$request->id_pemesanan' AND id_produk = '$request->id_produk' ");
                
                mysqli_query($conn,"UPDATE $this->detil SET sub_total_pemesanan = '$this->sub_total_pemesanan' WHERE id_pemesanan = '$request->id_pemesanan' AND id_produk = '$request->id_produk' ");
